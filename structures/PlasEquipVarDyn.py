from utils.trains import PlasEquipVarDynTrains
...
class AE_PlasEquipVarDyn(nn.Module, PlasEquipVarDynTrains):
    def __init__(self, **params):
        super(AE_PlasEquipVarDyn, self).__init__()
    ...
    def _set_default_params(self):
        ...
        defaults = {
            ...
            'batch_size': 2**14,
            'optimizer_plasvarequipdyn': None,
            'dropout_rate': 0,
            'gate_type': 'glu',
            'l1_weight': 0.01,
            'cos_weight': 1,
    ...
    def forward(self, X, L):
        ...
        if torch.isnan(Z).any():
            ...
            return None
        Z = Z.transpose(0,1)
        Z_hat = self.plasvarequipdyn(Z, L)
        if torch.isnan(Z_hat).any():
            print("NaN detected after PlasVarEquipDyn")
            return None
    ...
    def plasvarequipdyn(self,Z,V):
        for n, d in enumerate(self.params['layer_dimension']):
            V = self.PlasVarEquipDynDict[f'E{n}'](V)
        return self.PlasVarEquipDynDict['PlasVarEquipDyn'](Z.squeeze(1),V)
    
    def init_params(self):
        self.init_plasvarequipdyn()
        self.to(self.params['device'])

    def init_plasvarequipdyn(self):
        self.PlasVarEquipDynDict = nn.ModuleDict()
        in_dimension = self.params['process_variables']

        for n, d in enumerate(self.params['layer_dimension']):
            if n == len(self.params['layer_dimension'])-1:
                self.PlasVarEquipDynDict[f'E{n}'] = UnitCoder(
                    in_dimension, d, 
        ...
        self.PlasVarEquipDynDict['PlasVarEquipDyn'] = PlasVarEquipDyn(
    ...
    def _initialize_weights(self):
        for m in self.PlasVarEquipDynDict.modules():  # .modules()를 사용하여 모든 하위 모듈에 접근
    ...
    def l1_loss(self):
        total_params = sum(p.numel() for p in self.PlasVarEquipDynDict.parameters())
        l1_sum = sum(p.abs().sum() for p in self.PlasVarEquipDynDict.parameters())
        avg_l1 = l1_sum / total_params
        return torch.log1p(avg_l1)

class ResAE_PlasEquipVarDyn(AE_PlasEquipVarDyn):
    def __init__(self,**params):
        params['encoder'] = 'ResAutoencoder'
        super(ResAE_PlasEquipVarDyn, self).__init__(**params)

class LogAE_PlasEquipVarDyn(AE_PlasEquipVarDyn):
    def __init__(self,**params):
        params['encoder'] = 'LogAutoencoder'
        super(LogAE_PlasEquipVarDyn, self).__init__(**params)

class LogResAE_PlasEquipVarDyn(AE_PlasEquipVarDyn):
    def __init__(self,**params):
        params['encoder'] = 'LogResAutoencoder'
        super(LogResAE_PlasEquipVarDyn, self).__init__(**params)

class VariationalAE_PlasEquipVarDyn(AE_PlasEquipVarDyn):
    def __init__(self,**params):
        params['encoder'] = 'VariationalAutoencoder'
        super(VariationalAE_PlasEquipVarDyn, self).__init__(**params)

class VariationalResAE_PlasEquipVarDyn(AE_PlasEquipVarDyn):
    def __init__(self,**params):
        params['encoder'] = 'VariationalResAutoencoder'
        super(VariationalResAE_PlasEquipVarDyn, self).__init__(**params)

class VariationalLogAE_PlasEquipVarDyn(AE_PlasEquipVarDyn):
    def __init__(self,**params):
        params['encoder'] = 'VariationalLogAutoencoder'
        super(VariationalLogAE_PlasEquipVarDyn, self).__init__(**params)    

class VariationalLogResAE_PlasEquipVarDyn(AE_PlasEquipVarDyn):
    def __init__(self,**params):
        params['encoder'] = 'VariationalLogResAutoencoder'
        super(VariationalLogResAE_PlasEquipVarDyn, self).__init__(**params)